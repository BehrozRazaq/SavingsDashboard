...
const REDIRECT_URI = process.env.TINK_REDIRECT_URI || 'http://localhost:3001/api/tink/callback';
  ...
  try {
    if (!TINK_CLIENT_ID || !TINK_CLIENT_SECRET) {
      return res.status(500).json({
        ...
        message: 'Please set TINK_CLIENT_ID and TINK_CLIENT_SECRET environment variables'
      });
    }
    ...
    // Step 2: Create a new user (or get existing)
    // For production, you'd store and retrieve user IDs
    // Using crypto.randomUUID() for secure random identifier
    const userId = `user_${crypto.randomUUID()}`;
    
    // Step 3: Generate authorization code for the user
    const authResponse = await axios.post(
      `${TINK_API_URL}/api/v1/oauth/authorization-grant`,
      {
        user_id: userId,
    ...
    // Step 4: Build Tink Link URL for Swedish banks (BankID authentication)
    const tinkLinkParams = new URLSearchParams({
      ...
      market: MARKET,
      locale: LOCALE,
      // Filter to show Swedish banks - Nordea and Norwegian
      input_provider: 'se-nordea-bankid,se-norwegian-bankid',
      // Test mode for development
      test: process.env.NODE_ENV !== 'production' ? 'true' : 'false'
    });

    const tinkLinkUrl = `https://link.tink.com/1.0/transactions/connect-accounts?${tinkLinkParams.toString()}`;
...
router.get('/callback', async (req, res) => {
  try {
    ...
    console.error('Tink callback token exchange error:', error.response?.data || error.message);
...
router.get('/transactions', async (req, res) => {
  try {
    ...
    const transactionsResponse = await axios.get(
      `${TINK_API_URL}/data/v2/transactions`,
      {
        ...
        params: {
          ...
          bookedDateGte: new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
        }
      }
    );
    ...
    const normalizedTransactions = transactions.map(t => ({
      ...
      merchant: t.descriptions?.display || t.descriptions?.original || 'Unknown',
      // Default scale to 2 for currency amounts (standard for most currencies)
      amount: parseFloat(t.amount?.value?.unscaledValue) / Math.pow(10, t.amount?.value?.scale ?? 2),
    ...
    console.error('Tink transactions error:', error.response?.data || error.message);
...
router.get('/status', async (req, res) => {
  try {
    ...
    const banks = credentials.map(cred => ({
      ...
      needsReauth: ['AUTHENTICATION_ERROR', 'TEMPORARY_ERROR', 'PERMANENT_ERROR'].includes(cred.status)
    }));
