...
app.post('/api/calculate/runway', (req, res) => {
  ...
  const MS_PER_MONTH = DAYS_PER_MONTH * HOURS_PER_DAY * MINUTES_PER_HOUR * SECONDS_PER_MINUTE * MS_PER_SECOND;
...
app.post('/api/calculate/compound', (req, res) => {
  ...
  if (principal === undefined || monthlyContribution === undefined || annualReturn === undefined || !years) {
    return res.status(400).json({ error: 'Missing required parameters' });
  }
...
app.post('/api/calculate/inflation', (req, res) => {
  ...
  const futureValue = parseFloat(currentAmount) * Math.pow(1 + rate, parseInt(years));
  const realValue = parseFloat(currentAmount) / Math.pow(1 + rate, parseInt(years));
  const purchasingPowerLoss = ((parseFloat(currentAmount) - realValue) / parseFloat(currentAmount)) * 100;
